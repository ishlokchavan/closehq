...
export function GlassTabBar() {
  ...
  return (
    <nav
      ...
      aria-label="Primary"
    >
      <div className="lg-glass-strong lg-specular pointer-events-auto flex items-center gap-1 rounded-full px-2 py-2">
        {TABS.map(({ href, label, Icon, match }) => {
          ...
          return (
            <Link
              ...
              className={`relative flex h-12 items-center gap-2 rounded-full px-4 transition-all duration-300 ${
                active
                  ? 'bg-white/85 text-ink shadow-[0_4px_16px_rgba(0,0,0,0.18)]'
            ...
            >
              ...
              />
              <span
                className={`overflow-hidden text-[15px] font-medium tracking-tight transition-all duration-300 ${
              ...
              {showBadge && (
                <span className="absolute -right-0.5 -top-0.5 flex h-5 min-w-5 items-center justify-center rounded-full bg-journey-buyer px-1.5 text-[11px] font-semibold text-ink">
                  {savedRefs.length}
                </span>
              )}
            </Link>
